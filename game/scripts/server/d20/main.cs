...
function stripFileName(%fullPath)
{
    ...
    {
        %fileName = %fullPath;
        %fullPath = nextToken(%fullPath, token, "/");
    }
    return %fileName;
}

function loadD20(%dirPath)
{
...
loadD20(";feats;skills;equipment;classes;races;combat;player");

// Utility functions

/// <summary>
/// Rolls dice, returns the sum of results.
/// can use d1, d2, d3, d4, d6, d8, d10, d12, d20, d30 or d100, or 
/// simply specify a number of sides without the leading 'd'
/// </summary>
/// <param name="number">Number of dice to roll.</param>
/// <param name="type">Die type - simply specify number of sides.</param>
/// <return>Returns the sum of die roll results.</return>
function d20System::rollDice(%number, %type)
{
    %sides = strreplace(%type, "d", "");
    %sum = 0;
    for( %i = 0; %i < %number; %i++ )
    {
        %sum += getRandom(1, %sides);
    }
    return %sum;
}

/// <summary>
/// Rolls 4d6, drops the lowest, returns the sum of remaining 3.
/// </summary>
/// <return>Returns the sum of die roll results.</return>
function d20System::gen4d6Stat()
{
    %total = 0;
    %die1 = d20System::rollDice(1, d6);
    %die2 = d20System::rollDice(1, d6);
    %die3 = d20System::rollDice(1, d6);
    %die4 = d20System::rollDice(1, d6);
    %low = %die1;
    if( %die2 < %low )
        %low = %die2;
    if( %die3 < %low )
...
/// <summary>
/// Takes a space-delimited string and finds the index of a provided word
/// within the string data.
/// </summary>
/// <param name="data">A space-delimited string.</param>
/// <param name="search">A word to find in the data.</param>
/// <return>Returns the index of the desired string or -1 if not found.</return>
function d20System::getWordIndex(%data, %string)
{
...
/// <summary>
/// Takes a tab-delimited string of space-delimited fields containing 
/// Base Attack Bonus data and adds them to get the final BAB.
/// </summary>
/// <param name="data">A space-delimited string.</param>
/// <return>Returns a space-delimited string containing final BAB data.</return>
function d20System::addBAB(%data)
{
    %babCount = getFieldCount(%data);
    %finalBAB = "";
    for( %i = 0; %i < %babCount; %i++ )
    {
        %babData = getField(%data, %i);
        if( %finalBAB $= "" )
        {
            %finalBAB = %babData;
            continue;
        }
        %newBABCount = getWordCount(%babData);
        %currentAttackCount = getWordCount(%finalBAB);
        // loop for largest number of attacks
        %wordCount = ( %newBABCount > %currentAttackCount ? %newBABCount : %currentAttackCount );
        for( %j = 0; %j < %wordCount; %j++ )
        {
            %newBAB = getWord(%babData, %j);
            %currentBAB = getWord(%finalBAB, %j);
            if( %currentBAB $= "" )
            {
                %finalBAB = %finalBAB SPC %newBAB;
                continue;
            }
            else if( %newBAB $= "" )
            {
            ...
            {
                %currentBAB += %newBAB;
                %finalBAB = setWord(%finalBAB, %j, %currentBAB);
            }
        }
    }
    // now, roll the 5's forward...
    %finalCount = getWordCount(%finalBAB);
    for( %i = 0; %i < %finalCount; %i++ )
    {
        %bonus = getWord(%finalBAB, %i);
        %fiveCount = mFloor(%bonus / 5) - 1;
        if ( mFMod(%bonus, 5) > 0 )
            %fiveCount++;
        if( %fiveCount > 0 )
        {
            if( %i + 1 < %finalCount )
            {
                %nextBonus = getWord(%finalBAB, %i + 1) + %fiveCount;
                %finalBAB = setWord(%finalBAB, %i + 1, %nextBonus);
            }
            else
                %finalBAB = %finalBAB SPC %fiveCount;
        }
    }
    return %finalBAB;
}
