...
// Project13
// 3 Jan 2014
// d20 main equipment script loader
//-----------------------------------------------------------------------------
...
echo(" -- d20/equipment/main.cs");

function d20LoadEquipment()
{
    echo(" -- d20/equipment/main.cs : d20LoadEquipment()");
    // find main.cs
    %search = "./main.cs";
    %mainCs = "";
    ...
    %search = "*.cs";
    // search for scripts and load them
    %scripts = d20GetEquipmentFolders();
    while(%scripts !$= "")
    {
        %scripts = nextToken(%scripts, token, ";");
        %temp = %token;
        %temp = strreplace(%token, ".cs", "");
        if( %token !$= "" && %token $= %temp )
        {
            error("Found directory: " @ %token );
        }
        for( %file = findFirstFile( "./" @ %token @ %search ); %file !$= ""; %file = findNextFile( "./" @ %token @ %search ) )
        {
            %temp = %file;
            %temp = strreplace(%temp, ".cs", "");
            if( %temp !$= %file && %file !$= %mainCs )
                exec(%file);
        }
    }
}

function d20GetEquipmentFolders()
{
    %folderList = "";
    %search = "./*";
    for( %file = findFirstFile( %search ); %file !$= ""; %file = findNextFile( %search ) )
    {
        ...
        %temp = strreplace(%temp, ".cs", "");
        if( %temp $= %file )
            %folderList = %folderList @ stripFileName(%file) @ ";";
    }
    echo(" -- d20GetEquipmentFolders() : " @ %folderList);
    return %folderList;    
}
