...
echo(" -- d20/d20PlayerStats.cs");

/// <summary>
/// Takes an attribute value and returns the attribute modifier
/// </summary>
/// <param name="statValue">An attribute value.</param>
/// <return>Returns the attribute modifier, including bonus spell/ability slots per level.</return>
function d20System::getStatMod(%statValue)
{
    ...
    if(%statValue >= 22 && %statValue <= 23)
        return "+6	-	2	2	1	1	1	1	-	-	-";
    if(%statValue >= 24 && %statValue <= 25)
        return "+7	-	2	2	2	1	1	1	1	-	-";
    if(%statValue >= 26 && %statValue <= 27)
        return "+8	-	2	2	2	2	1	1	1	1	-";
    if(%statValue >= 28 && %statValue <= 29)
        return "+9	-	3	2	2	2	2	1	1	1	1";
    if(%statValue >= 30 && %statValue <= 31)
        return "+10	-	3	3	2	2	2	2	1	1	1";
    if(%statValue >= 32 && %statValue <= 33)
        return "+11	-	3	3	3	2	2	2	2	1	1";
    if(%statValue >= 34 && %statValue <= 35)
        return "+12	-	3	3	3	3	2	2	2	2	1";
    if(%statValue >= 36 && %statValue <= 37)
        return "+13	-	4	3	3	3	3	2	2	2	2";
    if(%statValue >= 38 && %statValue <= 39)
        return "+14	-	4	4	3	3	3	3	2	2	2";
    if(%statValue >= 40 && %statValue <= 41)
        return "+15	-	4	4	4	3	3	3	3	2	2";
    if(%statValue >= 42 && %statValue <= 43)
        return "+16	-	4	4	4	4	3	3	3	3	2";
    if(%statValue >= 44 && %statValue <= 45)
        return "+17	-	5	4	4	4	4	3	3	3	3";
    return "-1  -1";
}

function d20System::getAbilityScoreCost(%targetValue)
{
    switch(%targetValue)
    {
        case 7:
