...
echo(" -- d20/d20Player.cs");

function d20Player::setStats(%this, %str, %dex, %con, %int, %wis, %cha)
{
    %this.Character.Strength = %str;
    %this.Character.Dexterity = %dex;
    %this.Character.Constitution = %con;
    %this.Character.Intelligence = %int;
    %this.Character.Wisdom = %wis;
    %this.Character.Charisma = %cha;
    
    %this.StrengthBonus = d20System::getStatMod(%str);
    %this.DexterityBonus = d20System::getStatMod(%dex);
    %this.ConstitutionBonus = d20System::getStatMod(%con);
    %this.IntelligenceBonus = d20System::getStatMod(%int);
    %this.WisdomBonus = d20System::getStatMod(%wis);
    %this.CharismaBonus = d20System::getStatMod(%cha);
}

function d20Player::modifyStat(%this, %stat, %amount)
{
    switch(%stat)
    {
        case $Stat::Strength :
            %this.StatModStr += %amount;
            return;
        
        case $Stat::Dexterity :
            %this.StatModDex += %amount;
            return;

        case $Stat::Constitution :
            %this.StatModCon += %amount;
            return;

        case $Stat::Intelligence :
            %this.StatModInt += %amount;
            return;

        case $Stat::Wisdom :
            %this.StatModWis += %amount;
            return;

        case $Stat::Charisma :
            %this.StatModCha += %amount;
            return;
    }
}

function d20Player::modifyStatBonus(%this, %stat, %amount)
{
    switch(%stat)
    {
        case $Stat::Strength :
            %this.StatBonusModStr += %amount;
            return;
        
        case $Stat::Dexterity :
            %this.StatBonusModDex += %amount;
            return;

        case $Stat::Constitution :
            %this.StatBonusModCon += %amount;
            return;

        case $Stat::Intelligence :
            %this.StatBonusModInt += %amount;
            return;

        case $Stat::Wisdom :
            %this.StatBonusModWis += %amount;
            return;

        case $Stat::Charisma :
            %this.StatBonusModCha += %amount;
            return;
    }
}

function d20Player::getHealth(%this)
{
    return %this.currentHealth;
}

function d20Player::getMaxHealth(%this)
{
    return %this.Character.maxHealth;
}

function d20Player::getClass(%this)
{
    // run through the level list and total up the class levels possessed.
}

function d20Player::applyEffect(%this, %effect)
{
    // Apply effect - example effect:
    //$TableSideEffects::dRoll[0] = "01-08";
    //$TableSideEffects::SideEffect[0] = "Blurred Vision";
    //$TableSideEffects::SideEffectType[0] = "MissChance";
    //$TableSideEffects::SideEffectValue[0] = "Miss:20%";
}