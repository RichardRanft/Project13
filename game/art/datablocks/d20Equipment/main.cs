...
echo(" -- art/datablocks/d20Equipment/main.cs");

function d20Equipment::Load()
{
    // find main.cs
    %search = "./main.cs";
    %mainCs = "";
    ...
    {
        if (%file !$= %mainCs)
            %featScripts = %featScripts @ ";" @ stripFileName(%file);
    }
    while(%featScripts !$= "")
    {
        %featScripts = nextToken(%featScripts, token, ";");
        if(%token !$= "" && exec("./" @ %token) != true)
        {
